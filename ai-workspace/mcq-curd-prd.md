...
- [ ] Each of the five service methods has at least one passing test
- [ ] A create with six choices produces six rows with positions 1 through 6
- [ ] Zod rejects fewer than two choices, more than six, and any count of correct answers other than one
- [ ] `isCorrect` is a boolean on every returned choice
- [ ] No SQL is issued against these tables from outside the service
...
| `src/lib/validation/mcq-schemas.ts`             | Zod schemas, including the 2-to-6 and one-correct rules       | Phase 2             |
| `src/lib/validation/mcq-schemas.test.ts`        | Vitest: accept valid bodies, reject invalid choice sets       | Phase 2 (write first) |
| `src/lib/services/mcq-service.ts`               | All SQL against the three MCQ tables                          | Phase 2             |
| `src/lib/services/mcq-service.test.ts`          | Vitest: CRUD using `createFakeD1()`                           | Phase 2 (write first) |
...
**Status**: PLANNED
**Next Steps**: Write `mcq-schemas.test.ts` and `mcq-service.test.ts`, observe them fail, then implement the Zod schemas and the MCQ service.
